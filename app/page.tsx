...
const navItems = [
  ["Home", "/"], ["About", "/about"], ["Subjects", "/subjects"],
  ["Pricing", "/pricing"], ["Reading", "/reading"], ["Practice", "/practice"], ["Contact", "/contact"],
];
const subjects = [
...
export default function Home() {
  ...
  function submitForm(event: FormEvent<HTMLFormElement>) {
    ...
    // Replace this email handoff with Formspree, Resend, or another service when a backend is selected.
    if (event.currentTarget.checkValidity()) {
      const data = new FormData(event.currentTarget);
      const subject = encodeURIComponent(`Tutoring request from ${data.get("name")}`);
      const body = encodeURIComponent(`Name: ${data.get("name")}\nEmail: ${data.get("email")}\nPhone: ${data.get("phone")}\nGrade: ${data.get("grade")}\nCourse: ${data.get("course")}\nOption: ${data.get("option")}\nPreferred date: ${data.get("date")}\n\nMessage:\n${data.get("message")}`);
      window.location.href = `mailto:APMTutorings@gmail.com?subject=${subject}&body=${body}`;
      setSubmitted(true);
  ...
  return <>
    <header className="fixed inset-x-0 top-0 z-50 border-b border-slate-200/80 bg-white/95 backdrop-blur">
      <nav className="container-page flex h-20 items-center justify-between" aria-label="Main navigation">
        <Brand />
        <div className="hidden items-center gap-5 lg:flex">{navItems.map(([label,href]) => <a key={label} className="text-sm font-semibold text-slate-600 transition hover:text-brand-700" href={href}>{label}</a>)}</div>
        <a href="#contact" className="btn-primary hidden md:inline-flex">Book a Session <span className="ml-2">→</span></a>
        <button onClick={() => setMenuOpen(!menuOpen)} className="grid h-11 w-11 place-items-center rounded-lg border border-slate-200 lg:hidden" aria-expanded={menuOpen} aria-controls="mobile-menu" aria-label="Toggle navigation menu"><span className="text-2xl">{menuOpen ? "×" : "☰"}</span></button>
      </nav>
      {menuOpen && <div id="mobile-menu" className="border-t border-slate-100 bg-white px-5 pb-6 pt-3 shadow-lg lg:hidden">{navItems.map(([label,href]) => <a onClick={() => setMenuOpen(false)} key={label} className="block border-b border-slate-100 py-3 font-semibold" href={href}>{label}</a>)}<a onClick={() => setMenuOpen(false)} href="#contact" className="btn-primary mt-5 w-full">Book a Session</a></div>}
    </header>
    ...
    <main>
      ...
      <section className="section-pad bg-brand-50/70"><div className="container-page"><div className="text-center"><p className="eyebrow">Testimonials</p><h2 className="section-title">The kind of progress families value</h2><p className="section-copy mx-auto">Sample testimonial copy is shown below and can be replaced with verified customer feedback.</p></div><div className="mt-12 grid gap-5 md:grid-cols-3">{["The explanations are patient and easy to follow—math finally feels manageable.","I feel more confident starting difficult assignments and asking questions.","Affordable, thoughtful support made a positive difference in our student’s routine."].map((x,i)=><article key={i} className="rounded-2xl border border-brand-100 bg-white p-7 shadow-card"><div className="text-gold-400" aria-hidden="true">☆ ☆ ☆ ☆ ☆</div><p className="mt-6 text-lg font-bold leading-7 text-navy-900">“{x}”</p><p className="mt-5 text-xs font-bold uppercase tracking-wider text-slate-400">Sample testimonial — not customer feedback</p></article>)}</div></div></section>
      ...
      <section id="contact" className="section-pad bg-slate-50"><div className="container-page grid gap-10 lg:grid-cols-[.75fr_1.25fr]"><div><p className="eyebrow">Book a session</p><h2 className="section-title">Let’s make math feel manageable</h2><p className="section-copy">Tell us what support you’re looking for. We’ll follow up to discuss availability and confirm your session.</p><div className="mt-9 space-y-4 rounded-2xl bg-navy-950 p-7 text-white"><p><span className="mr-3 text-brand-100">✉</span><span className="text-slate-400">Email</span><br/><span className="ml-8 font-bold">APMTutorings@gmail.com</span></p><p><span className="mr-3 text-brand-100">☎</span><span className="text-slate-400">Phone</span><br/><span className="ml-8 font-bold">469-234-1396</span></p><p><span className="mr-3 text-brand-100">⌖</span><span className="text-slate-400">Location</span><br/><span className="ml-8 font-bold">Remote</span></p></div></div>
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-card sm:p-9">{submitted ? <div className="grid min-h-[500px] place-items-center text-center" role="status"><div><span className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-emerald-100 text-3xl text-emerald-700">✓</span><h3 className="mt-6 text-2xl font-extrabold">Thank you!</h3><p className="mx-auto mt-3 max-w-md leading-7 text-slate-600">Your email app has opened with your request addressed to APMTutorings@gmail.com. Send the prepared email and APM Tutoring will contact you soon.</p><button className="btn-secondary mt-7" onClick={()=>setSubmitted(false)}>Send another request</button></div></div> : <form onSubmit={submitForm} className="grid gap-5 sm:grid-cols-2">
    ...
    <footer className="bg-navy-950 py-12 text-white"><div className="container-page"><div className="grid gap-10 border-b border-white/10 pb-10 md:grid-cols-3"><div><Brand inverted/><p className="mt-5 max-w-xs text-sm leading-6 text-slate-400">Helping students build stronger math skills.</p></div><div><p className="font-bold">Quick links</p><div className="mt-4 grid grid-cols-2 gap-3">{navItems.map(([label,href])=><a key={label} href={href} className="text-sm text-slate-400 hover:text-white">{label}</a>)}</div></div><div><p className="font-bold">Contact</p><div className="mt-4 space-y-2 text-sm text-slate-400"><p>APMTutorings@gmail.com</p><p>469-234-1396</p><p>Remote</p></div></div></div><div className="flex flex-col gap-2 pt-7 text-xs text-slate-500 sm:flex-row sm:justify-between"><p>© {new Date().getFullYear()} APM Tutoring. All rights reserved.</p><p>Affordable support. Stronger skills. Greater confidence.</p></div></div></footer>
  </>;
}
