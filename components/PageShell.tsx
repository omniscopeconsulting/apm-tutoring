...
export function PageShell({ eyebrow, title, intro, children }: { eyebrow: string; title: string; intro: string; children: ReactNode }) {
  return <div className="min-h-screen bg-slate-50">
    <header className="border-b border-slate-200 bg-white"><nav className="container-page flex min-h-20 flex-wrap items-center justify-between gap-4 py-4" aria-label="Main navigation"><Link href="/" className="flex items-center gap-2 font-extrabold"><span className="grid h-10 w-10 place-items-center rounded-xl bg-navy-950 text-xs text-white">APM</span><span>APM <span className="text-brand-600">Tutoring</span></span></Link><div className="flex flex-wrap items-center justify-end gap-x-5 gap-y-2">{links.map(([label, href]) => <Link className="text-sm font-semibold text-slate-600 hover:text-brand-700" key={href} href={href}>{label}</Link>)}</div></nav></header>
